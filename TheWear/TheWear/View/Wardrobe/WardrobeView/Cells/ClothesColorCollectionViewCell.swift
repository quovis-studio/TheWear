...
class ClothesColorCollectionViewCell: UICollectionViewCell, UIGestureRecognizerDelegate {
    
    var colorChoosen: ((Int) -> ())?
    
    var choosenBorderView: UIView!
    
    func updateColor(with color: Color.clothes, choosen: Bool) {
        if color == .white {
            backgroundColor = Color.clothes.whiteForWardrobe.color
        } else {
