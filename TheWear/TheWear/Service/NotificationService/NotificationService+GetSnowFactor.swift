...
extension NotificationService {
    ...
    func getSnowFactor(
        ...
        if dayOffset == 0 {
            ...
            // Границы индекса задаются динамически, так как в первом дне может быть меньше чем 24 часа
            if firstHour < 6 {
                ...
                day = (dayToUse[12 - firstHour...(dayToUseHoursCount <= 18 ? dayToUseHoursCount - 1 : 18)].map {
                    ($0.weather?.icon?.prefix(1) ?? "0").contains("s") ? 1 : 0
                }.reduce(0, +)) > 0 ? 1 : 0
                evening = (dayToUse[19 - firstHour...(dayToUseHoursCount <= 23 ? dayToUseHoursCount - 1 : 23 - firstHour)].map {
                    ($0.weather?.icon?.prefix(1) ?? "0").contains("s") ? 1 : 0
